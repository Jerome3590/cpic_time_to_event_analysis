...
This project calculates scaled feature importance for predicting fall injury and ED visit outcomes using:
...
│    - Target: fall_injury_any (falls) or ed_event (ed)       │
└─────────────────────────────────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────────┐
...
3. **Recall as Quality Metric**
   - Appropriate for imbalanced classification (fall injury, ED event)
...
- `falls_0_12_catboost_feature_importance.csv`
- `falls_0_12_xgboost_feature_importance.csv`
- `falls_0_12_xgboost_rf_feature_importance.csv`
- `falls_0_12_constant_features.csv`
...
**Example Files:**
- `falls_0_12_top50_features.png`
- `falls_0_12_top50_with_recall.png`
- `falls_0_12_normalized_vs_scaled.png`
- `falls_0_12_category_distribution.png`
...
For the `falls` cohort, age band `65-74`:
- **Total Z codes:** 386
...
**Analysis Results for `falls` cohort, age band `65-74`:**
...
features <- read_csv("falls_65-74_2016_feature_importance_aggregated.csv")
...
```r
# Load both cohorts
falls_fi <- read_csv("falls_65-74_2016_feature_importance_aggregated.csv")
ed_fi    <- read_csv("ed_65-74_2016_feature_importance_aggregated.csv")

# Find common features
common_features <- intersect(
  head(falls_fi, 50)$feature,
  head(ed_fi, 50)$feature
)

# Find falls-specific features
falls_specific <- setdiff(
  head(falls_fi, 50)$feature,
  head(ed_fi, 50)$feature
)
```
...
- **Cons:** **Longer runtime** (e.g., ~11–12 hours for large cohorts such as `falls, 25–44` with 25 MC-CV splits and 3 core models).
...
**Large Cohort (e.g., falls, 25-44 age band):**
...
### falls Age-Band Size and Runtime Scaling (N_SPLITS = 25)

Using cohort parquets in `gold/cohorts/cohort_name=falls/` (65-74 and 75-84 only, train = 2016–2018, test = 2019):

- **Event-level row counts (workload):**
  - **65–74**: train = 2,857,618, test = 1,015,348 (baseline)
  - **75–84**: train = 1,227,068, test = 370,364 (≈ 0.43×)

- **Distinct patients:**
  - **65–74**: train = 23,356, test = 9,150
  - **75–84**: train = 8,477, test = 2,976

**falls 65–74** is the heavier partition. Both run sequentially; expect **75–84** to take roughly half the time of **65–74**.

### ed Age-Band Size and Runtime Scaling (N_SPLITS = 25)

For the ed cohort (`cohort_name=ed`), cohort parquets in `gold/cohorts/cohort_name=ed/` (65-74 and 75-84 only):

- **Event-level row counts (workload), train = 2016–2018, test = 2019:**
  - **65–74**: train = 135,465,040, test = 50,047,383 (baseline)
  - **75–84**: train = 87,267,781, test = 32,780,611 (≈ 0.65×)

- **Distinct patients, train = 2016–2018, test = 2019:**
  - **65–74**: train = 919,654, test = 766,298
  - **75–84**: train = 462,222, test = 391,003

**ed 65–74 is the heaviest partition** (baseline = 1.0×); **75–84 ≈ 0.65×** by event workload. Both run sequentially per the EC2 runbook.
...
**For Large Cohort (ed, 65-74 age band, 25 MC-CV splits):**
