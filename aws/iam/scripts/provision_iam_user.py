#!/usr/bin/env python3
"""Provision a scoped IAM user for CPIC time-to-event artifact access."""
...
DEFAULT_POLICY_FILE = (
    ...
    / "policies"
    / "cpic-time-to-event-artifact-access-policy-v2.json"
)
...
def set_login_profile(iam, username: str, password: str, execute: bool) -> None:
    if not execute:
        print(f"DRY RUN: would configure login profile for {username} with password reset required")
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--policy-arn",
        default=None,
        help="Optional existing policy ARN to attach instead of creating the scoped CPIC policy.",
    )
